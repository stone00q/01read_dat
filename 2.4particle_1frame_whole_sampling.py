...
def split_particles_by_Q(particles, q_array):
    ...
    for i,particle in enumerate(particles):
        ...
        num_neighbors = len(kdtree.query_ball_point(particle.position, 2 * gen_radius))
         # 根据局部密度调整分裂概率
        if num_neighbors > 30:  # 若周围有超过 10 个粒子，则降低分裂概率
            split_prob *= 0.2  # 只保留 20% 的原始概率
        elif num_neighbors > 50:  # 若粒子非常密集，则完全不分裂
            continue
...
def main():
    # 全局变量
    global GLOBAL_GRID, GLOBAL_PROBE_FILTER,GLOBAL_CELL_LOCATOR,Q_MAX,GLOBAL_VALID_ARRAY,GLOBAL_BOUNDS

    # 设置粒子保存路径
    particle_output_dir = r"D:\Project\01read_dat\data_new\steps=150-nump=100000"
    # 确保目录存在
    ...
    GLOBAL_BOUNDS=GLOBAL_GRID.GetBounds()
    print("bounds:{GLOBAL_BOUNDS[0]} {GLOBAL_BOUNDS[1] {GLOBAL_BOUNDS[2]}}")
     # 新增随机点生成代码
    # 生成100000个在全局边界内的随机点
    num_random_points = 100000
    random_points = np.zeros((num_random_points, 3))
    
    # 为每个坐标轴生成随机值
    for i in range(3):
        min_val = GLOBAL_BOUNDS[2*i]
        max_val = GLOBAL_BOUNDS[2*i + 1]
        random_points[:, i] = np.random.uniform(min_val, max_val, num_random_points)
    # 将随机点转换为粒子实例
    particles = [Particle(pos, energy_max, 0) for pos in random_points]
